...
import { DB_REFRESH_SESSIONS, DB_USERS, SupaQuery } from '@lib/server/db';
import { buildInsertQuery, buildUpdateQuery, FetchMode } from '@lib/server/db';
...
export async function authenticateNativeSupabaseUser(email: string, plainPassword: string): Promise<UserDTO> {
  ...
  const { data: userProfile, error: fetchError } = await new SupaQuery<UserWithPassword>(DB_USERS)
...
export async function authenticateSocialSupabaseUser(providerType: ProviderType, providerId: string): Promise<UserDTO> {
  ...
  const { data, error: fetchError } = await new SupaQuery<UserDTO>(DB_USERS)
