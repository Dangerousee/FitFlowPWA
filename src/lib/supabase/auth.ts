...
import { AccountStatus, LoginType, ProviderType } from '@enums/auth';
import * as ErrorCodes from '@constants/errorCodes';
import bcrypt from 'bcryptjs';
import camelcaseKeys from 'camelcase-keys';
import { LoginRequestBody } from '@models/auth.model';

export async function authenticateSupabaseUser(body: LoginRequestBody): Promise<SupabaseUserModel> {
  if (body.loginType === LoginType.NATIVE) {
    const { email, password } = body;
    return authenticateNativeSupabaseUser(email, password);
  }

  if (body.loginType === LoginType.SOCIAL) {
    const { providerType, providerId } = body;
    return authenticateSocialSupabaseUser(providerType, providerId);
  }

  console.error(`알 수 없는 로그인 타입`, body);
  throw {
    statusCode: 500,
...
export async function authenticateSocialSupabaseUser(providerType: ProviderType | undefined, providerId: string | undefined): Promise<SupabaseUserModel> {
  ...
  if (fetchError || !userProfile) {
    ...
    console.warn(`로그인 시도 실패 (${errMessage}): 사용자를 찾을 수 없거나 DB 오류 발생.`, fetchError?.message);
    throw {
  ...
  if (!userProfile.provider_type || !userProfile.provider_id) {
  ...
  const isTypeMatch = providerType === userProfile.provider_type;
  const idMatch = providerId === userProfile.provider_id;
...
/** Supabase 사용자 인증 처리 */
export async function authenticateNativeSupabaseUser(email: string | undefined, plainPassword: string | undefined): Promise<SupabaseUserModel> {
  type UserWithPassword = SupabaseUserModel & { password?: string };
 ...
 */
export async function updateUserLoginDetails(userId: string, timestamp: string): Promise<SupabaseUserModel> {
